...
def _run_single_experiment(name, allowed_snrs, focus_class=None):
    print("=" * 80)
    print(f"[Random Forest] 실험: {name} | 허용 SNR: {allowed_snrs}")
    print("=" * 80)
    ...
    # 스케일 표준화 + Random Forest 파이프라인
    rf_pipeline = Pipeline([
        ('scaler', StandardScaler()),
        ('rf', RandomForestClassifier(**RF_CONFIG, class_weight='balanced'))
    ])
    ...
    model_path = Path(MODEL_DIR) / f"rf_model_{name}.pkl"
...
def run_all_experiments():
    ...
    with open(summary_path, 'w', encoding='utf-8') as f:
        f.write("=" * 100 + "\n")
        f.write("Random Forest 모델 실험 종합 요약\n")
        f.write("=" * 100 + "\n\n")
        
        # 모델 설정 정보
        f.write("[모델 설정]\n")
        f.write(f"  알고리즘: Random Forest\n")
        f.write(f"  하이퍼파라미터: {RF_CONFIG}\n")
        f.write(f"  전처리: StandardScaler 적용\n")
        f.write(f"  클래스 가중치: balanced\n\n")
