...
def create_classification_dataset_fixed(data_dir, output_dir, window_size=WINDOW_SIZE,
    ...
    # SNR 분포 확인 (훈련/테스트)
    def count_snr_distribution(snr_array):
        snr_counts = {}
        for val in snr_array:
            if val is None or (isinstance(val, float) and np.isnan(val)):
                key = '원본'
            else:
                key = f'{float(val):.0f}dB'
            snr_counts[key] = snr_counts.get(key, 0) + 1
        return snr_counts
    
    train_snr_dist = count_snr_distribution(snr_train)
    test_snr_dist = count_snr_distribution(snr_test)
    print(f"훈련 SNR 분포: {train_snr_dist}")
    print(f"테스트 SNR 분포: {test_snr_dist}")
