...
from config import DATA_DIR, OUTPUT_DIR, MODEL_DIR, SVM_CONFIG
...
def train_svm_model():
    ...
    svm = SVC(C=10, gamma='scale', class_weight=class_weight, kernel='rbf')
    print("모델 학습 중...")
    ...
    with open(results_path, 'w', encoding='utf-8') as f:
        f.write("=" * 70 + "\n")
        f.write("SVM 모델 학습 결과\n")
        f.write("=" * 70 + "\n\n")
        f.write("[모델 파라미터]\n")
        f.write(f"C: 10\n")
        f.write(f"Gamma: scale\n")
        f.write(f"Kernel: rbf\n")
        f.write(f"Class Weight: {str(class_weight)}\n\n")
        f.write("[성능 지표]\n")
