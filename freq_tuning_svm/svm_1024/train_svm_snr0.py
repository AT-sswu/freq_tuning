...
def _run_single_experiment(name, allowed_snrs, focus_class=None):
    print("=" * 80)
    print(f"[SVM] 실험: {name} | 허용 SNR: {allowed_snrs}")
    print("=" * 80)
    ...
    svm_pipeline = Pipeline([
        ...
        ('svc', SVC(**SVM_CONFIG, class_weight='balanced'))
    ])
    ...
    model_path = Path(MODEL_DIR) / f"svm_model_{name}.pkl"
...
def run_all_experiments():
    ...
    with open(summary_path, 'w', encoding='utf-8') as f:
        f.write("=" * 100 + "\n")
        f.write("SVM 모델 실험 종합 요약\n")
        f.write("=" * 100 + "\n\n")
        
        # 모델 설정 정보
        f.write("[모델 설정]\n")
        f.write(f"  알고리즘: Support Vector Machine (SVM)\n")
        f.write(f"  하이퍼파라미터: {SVM_CONFIG}\n")
