...
def train_xgb_0db():
    """XGBoost 모델 SNR 0dB 고정 학습 및 주파수별 성능 분석"""
    print("=" * 70)
    print("XGBoost SNR 0dB 고정 학습 (주파수별 성능 분석)")
    print("=" * 70)
    ...
    # 모델 저장
    model_filename = Path(MODEL_DIR) / "xgb_snr_0db_fixed.pkl"
    joblib.dump(model, model_filename)
    print(f"\n✓ 모델 저장: {model_filename}")
    
    # 결과 저장
    results_df = pd.DataFrame(results)
    results_csv = Path(OUTPUT_DIR) / "xgb_0db_frequency_results.csv"
