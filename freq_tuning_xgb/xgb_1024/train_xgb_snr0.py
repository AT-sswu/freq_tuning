...
def _run_single_experiment(name, allowed_snrs, focus_class=None):
    print("=" * 80)
    print(f"[XGBoost] 실험: {name} | 허용 SNR: {allowed_snrs}")
    print("=" * 80)
    ...
    # 스케일 표준화 + XGBoost 파이프라인
    xgb_pipeline = Pipeline([
        ('scaler', StandardScaler()),
        ('xgb', xgb.XGBClassifier(**XGBOOST_CONFIG))
    ])
    ...
    model_path = Path(MODEL_DIR) / f"xgb_model_{name}.pkl"
...
def run_all_experiments():
    ...
    with open(summary_path, 'w', encoding='utf-8') as f:
        f.write("=" * 100 + "\n")
        f.write("XGBoost 모델 실험 종합 요약\n")
        f.write("=" * 100 + "\n\n")
        
        # 모델 설정 정보
        f.write("[모델 설정]\n")
        f.write(f"  알고리즘: XGBoost (Extreme Gradient Boosting)\n")
        f.write(f"  하이퍼파라미터: {XGBOOST_CONFIG}\n")
        ...
        for idx, s in enumerate(summaries, 1):
            ...
            # 혼돐 행렬
            f.write("[혼돐 행렬 (Confusion Matrix)]\n")
